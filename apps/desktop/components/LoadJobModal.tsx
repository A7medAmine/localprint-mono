...
const LoadJobModal: React.FC<LoadJobModalProps> = ({ isOpen, onClose, onSelect, acceptType: _acceptType, filterType = "all" }) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      ...
      try {
        ...
        let list = Array.isArray(data) ? data.filter((j: PrintJob) => j.serverFileName) : [];
        if (filterType === "image") {
          list = list.filter((j: PrintJob) => j.fileType?.startsWith("image/"));
        } else if (filterType === "pdf") {
          list = list.filter((j: PrintJob) => j.fileType === "application/pdf");
        }
        setJobs(list);
      } catch {
  ...
  const handleSelect = async (job: PrintJob) => {
          ...
          {loading ? (
            ...
            (() => {
              ...
              return Object.entries(groups).map(([customer, customerJobs]) => (
                <div key={customer}>
                  <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider px-1 py-2 sticky top-0 bg-card z-10 border-b border-border">
                  ...
                  {customerJobs.map((job) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground">{job.pageCount ? `${job.pageCount} ${t("pages")}` : ""}</div>
                    </button>
                  ))}
                </div>
              ));
            ...
            jobs.map((job) => (
              <button
                key={job.id}
                disabled={downloading === job.id}
                className="w-full flex items-center gap-4 p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition disabled:opacity-50 text-start"
                onClick={() => handleSelect(job)}
              >
                <div className="w-10 h-10 bg-muted rounded-lg flex items-center justify-center shrink-0">
                  <Icon name="file-doc" className="w-5 h-5 text-muted-foreground" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-foreground truncate">{job.fileName}</div>
                ...
                <div className="text-end shrink-0">
                  <span className={`inline-block text-xs font-medium px-2 py-0.5 rounded-full ${
                    ...
                    job.status === "READY" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" :
                    "bg-muted text-muted-foreground"
                  }`}>
                    {job.status}
                  </span>
                  <div className="text-xs text-muted-foreground mt-0.5">{job.pageCount ? `${job.pageCount} ${t("pages")}` : ""}</div>
                </div>
              </button>
            ))
          )}
