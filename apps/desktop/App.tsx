...
const App: React.FC = () => {
  ...
  const [lang, setLang] = useState<Language>(() => {
    return readEnumPref<Language>("language", ["en", "ar"], "ar");
  });
  ...
  const [themeMode, setThemeMode] = useState<ThemeMode>(() => {
    ...
    const legacy = readPref("legacyDarkMode");
  ...
  useEffect(() => {
    ...
    return onAppEvent("session-expired", onSessionExpired);
  ...
  useEffect(() => {
    ...
    return onAppEvent("must-change-password", onMustChangePassword);
  ...
  const handleLogout = () => {
    fetch("/api/auth/logout", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${readPref("adminToken")}`,
      },
    }).catch(() => {});
    storageService.setAuthToken(null);
    clearPref("adminToken");
