...
export const StatusBadge: React.FC<{ job: PrintJob }> = ({ job }) => {
  const { t } = useAdmin();
  ...
  return (
    ...
    >
      <Icon name={printed ? "check-circle" : ready ? "check" : "clock"} className="w-3 h-3" />
      {printed ? t("printed") : ready ? t("ready") : t("pending")}
    </span>
  );
};
