...
const AdminViewInner: React.FC<AdminViewProps> = ({
  ...
  // Paper types / printer prefs are owned by SettingsPanel's draft; the rest of
  // the dashboard reads the *saved* values off currentSettings.
      ...
      <div className="min-h-0">
        {activeTab === "jobs" ? (
          ...
          <SettingsPanel
            ...
            jobStats={{
              pending: groups.reduce((acc, g) => acc + g.jobs.filter((j) => j.status === PrintStatus.PENDING).length, 0),
              ready: groups.reduce((acc, g) => acc + g.jobs.filter((j) => j.status === PrintStatus.READY).length, 0),
              printed: groups.reduce((acc, g) => acc + g.jobs.filter((j) => j.status === PrintStatus.PRINTED).length, 0),
